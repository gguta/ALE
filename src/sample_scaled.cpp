...
//The general structure of the calculation, and lot of the code, is the same as p(ale) cf. model.cpp.
//(this could be made more clear)
string exODT_model::sample(bool S_node, long int g_id, int t_i, scalar_type rank, int e, scalar_type branch_length,
                           string branch_events, string transfer_token, bool max_rec) {
    ...
    if (rank > -1) {
        ...
        if (e == alpha) {
            ...
            if (S_node)//and 0!?
            {
                ...
                for (int branch_i=0;branch_i<n;branch_i++)
              {
                ...
                */
                ...
                if (not is_a_leaf)
                    for (int i = 0; i < N_parts; i++) {
                        ...
                        scalar_type Sb = 2 * sigma_hat * Delta_t *
                                         (qvec[gp_id + 1][rank][t_i][alpha] * qvec[gpp_id + 1][rank][t_i][alpha]) * pp;
                ...
                //0 EVENT
                scalar_type empty =
                        (1 + (delta_avg + tau_avg - lambda_avg - ni * sigma_hat - 2 * sigma_hat * H_hat) * Delta_t) *
            ...
            if (S_node) {
                ...
                scalar_type empty = (1 + (2 * delta_e * Eet - sigma_hat * H_hat - delta_e - lambda_e) * Delta_t) *
    ...
    if (ale_pointer->Bip_counts.count(g_id) and ale_pointer->Bip_counts[g_id] > 0) {
        new_branch_length = max(ale_pointer->Bip_bls[g_id] / ale_pointer->Bip_counts[g_id],
                                (scalar_type) scalar_parameter["min_branch_lenghts"]);
    } else {
        new_branch_length = max(ale_pointer->Bip_bls[g_id] / ale_pointer->observations,
                                (scalar_type) scalar_parameter["min_branch_lenghts"]);
    ...
    if (back_step.event == "D" or back_step.event == "Tb" or back_step.event == "S" or back_step.event == "Sb") {
        ...
        if (back_step.event == "S") {
            ...
            if (back_step.event == "Tb") {
                ...
                branch_string << branch_events << back_step.event << "@" << back_step.rank << "|" << named_branch.str()
                ...
                branch_string << branch_events << "T@" << rank << "|" << named_branch.str() << ":"
                              << max(new_branch_length, (scalar_type) 0.0);
            } else {
                ...
                branch_string << branch_events << back_step.event << "@" << rank << "|" << named_branch.str() << ":"
                              << max(new_branch_length, (scalar_type) 0.0);
            }
        }
    ...
    } else if (back_step.event == "TLb" or back_step.event == "SL" or back_step.event == "SLb" or
               back_step.event == "0") {
        ...
        if (transfer_token_stream.str() == "") {
            return sample(S_node, back_step.g_id, t_i + (back_step.event == "SL"), rank, back_step.e, new_branch_length,
                          branch_string.str(), transfer_token, max_rec);
        } else {
            return sample(S_node, back_step.g_id, t_i + (back_step.event == "SL"), rank, back_step.e, new_branch_length,
                          branch_string.str(), transfer_token_stream.str(), max_rec);
        }
    } else {
        cout << "error " << endl;
        cout << " g_id " << g_id;
