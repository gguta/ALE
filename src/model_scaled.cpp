...
//        string gene_name=ale->id_leaves[ g_id ];
            ...
            boost::split(tokens, gene_name, boost::is_any_of(string_parameter["gene_name_separators"]),
        ...
        for (int rank = 0; rank < last_rank; rank++) {
            ...
            for (int t_i = 0; t_i < (int) time_slice_times[rank].size(); t_i++) {
                ...
                if (t == 0) {
                    ...
                    print_simple(g_id, rank, t_i, alpha, 0, "1t=0/alpha ");
                }
                    ...
                    for (int branch_i = 0; branch_i < n; branch_i++) {
                        ...
                        if (t == 0) {
                            if (is_a_leaf && extant_species[e] == gid_sps[g_id]) {
                                ...
                                print_simple(g_id, rank, t_i, e, 1, "2t=0/e     ");
                            } else {
                                qvec[g_id + 1][rank][t_i][e] = 0;
                                print_simple(g_id, rank, t_i, e, 0, "2t=0/e     ");
                            }
                        }
                            ...
                            if (branch_i == n - 1 && rank > 0) {
                                ...
                                if (not is_a_leaf)
                                    for (int i = 0; i < N_parts; i++) {
                                        ...
                                        scalar_type S_pf_ppg =
                                                qvec[gp_id + 1][rank][t_i][f] * qvec[gpp_id + 1][rank][t_i][g] * pp;
                                        scalar_type S_ppf_pg =
                                                qvec[gpp_id + 1][rank][t_i][f] * qvec[gp_id + 1][rank][t_i][g] * pp;
                                ...
                                if (q_sum < EPSILON) {
                                    ...
                                    print_simple(g_id, rank, t_i, e, EPSILON, "3t_i=0/EPSI");
                                } else {
                                    qvec[g_id + 1][rank][t_i][e] = q_sum;
                                    print_simple(g_id, rank, t_i, e, q_sum, "3t_i=0/q_su");
                                }
                            }
                ...
                if (1) {
                    ...
                    print_simple(g_id, tpdt_rank, tpdt_t_i, alpha, 0, "4alpha virt");
                    scalar_type q_sum = 0;
                    ...
                    if (not is_a_leaf)
                        for (int i = 0; i < N_parts; i++) {
                            ...
                            scalar_type Sb = 2 * sigma_hat * Delta_t *
                                             (qvec[gp_id + 1][rank][t_i][alpha] * qvec[gpp_id + 1][rank][t_i][alpha]) *
                                             pp;
                    ...
                    scalar_type empty = (1 +
                                         (delta_avg + tau_avg - lambda_avg - ni * sigma_hat - 2 * sigma_hat * H_hat) *
                                         Delta_t) * qvec[g_id + 1][rank][t_i][alpha];
                    ...
                    if (qvec[g_id + 1][tpdt_rank][tpdt_t_i][alpha] < EPSILON) {
                        ...
                        print_simple(g_id, tpdt_rank, tpdt_t_i, alpha, EPSILON, "6alpha EPSI");
                    }
                ...
                if (1) {
                    for (int branch_i = 0; branch_i < n; branch_i++) {
                        ...
                        print_simple(g_id, tpdt_rank, tpdt_t_i, e, 0, "7time=t    ");
                        scalar_type q_sum = 0;
                        ...
                        if (not is_a_leaf)
                            for (int i = 0; i < N_parts; i++) {
                                ...
                                scalar_type Sb_pa_ppe =
                                        sigma_hat * Delta_t * qvec[gp_id + 1][rank][t_i][alpha] * qppe * pp;
                                scalar_type Sb_pe_ppa =
                                        sigma_hat * Delta_t * qpe * qvec[gpp_id + 1][rank][t_i][alpha] * pp;
                        ...
                        scalar_type empty =
                                (1 + (2 * delta_e * Eet - sigma_hat * H_hat - delta_e - lambda_e) * Delta_t) *
                        ...
                        if (qvec[g_id + 1][tpdt_rank][tpdt_t_i][e] < EPSILON) {
                            ...
                            print_simple(g_id, tpdt_rank, tpdt_t_i, e, EPSILON, "9e+=EPSILON");
                        }
    ...
    for (int rank = 0; rank < last_rank; rank++)
        for (int tsi = 0; tsi < (int) time_slice_times[rank].size(); tsi++) {
            ...
            for (int ii = 0; ii < scalar_parameter["DD"]; ii++) {
                ...
                // k1 = f(y[n])
                E_k1[-1] = (-sigma_hat) * Ee_y[-1] * Ee_y[-1] * h +
                           (delta_avg + tau_avg - lambda_avg - sigma_hat * ni) * Ee_y[-1] * h;
                ...
                for (int i = 0; i < (int) time_slices[rank].size(); i++) {
                    ...
                    // k1 = f(y[n])
                    E_k1[e] = delta * Ee_y[e] * Ee_y[e] * h + (-sigma_hat * Ee_y[-1] - delta - lambda) * Ee_y[e] * h +
                              lambda * h;
                }
                // k2 = f(y[n]+h/2 k1)
                ...
                E_k2[-1] = (-sigma_hat) * Ee_y[-1] * Ee_y[-1] * h +
                           (delta_avg + tau_avg - lambda_avg - sigma_hat * ni) * Ee_y[-1] * h;
                ...
                for (int i = 0; i < (int) time_slices[rank].size(); i++) {
                    ...
                    E_k2[e] = delta * Ee_y[e] * Ee_y[e] * h + (-sigma_hat * Ee_y[-1] - delta - lambda) * Ee_y[e] * h +
                              lambda * h;
                }
                ...
                E_k3[-1] = (-sigma_hat) * Ee_y[-1] * Ee_y[-1] * h +
                           (delta_avg + tau_avg - lambda_avg - sigma_hat * ni) * Ee_y[-1] * h;
                for (int j = 0; j < (int) time_slices[rank].size(); j++) {
                    int f = time_slices[rank][j];
                    scalar_type tau_f = vector_parameter["tau"][f];
                ...
                for (int i = 0; i < (int) time_slices[rank].size(); i++) {
                    ...
                    E_k3[e] = delta * Ee_y[e] * Ee_y[e] * h + (-sigma_hat * Ee_y[-1] - delta - lambda) * Ee_y[e] * h +
                              lambda * h;
                }
                ...
                E_k4[-1] = (-sigma_hat) * Ee_y[-1] * Ee_y[-1] * h +
                           (delta_avg + tau_avg - lambda_avg - sigma_hat * ni) * Ee_y[-1] * h;
                for (int j = 0; j < (int) time_slices[rank].size(); j++) {
                    int f = time_slices[rank][j];
                    scalar_type tau_f = vector_parameter["tau"][f];
                ...
                for (int i = 0; i < (int) time_slices[rank].size(); i++) {
                    ...
                    E_k4[e] = delta * Ee_y[e] * Ee_y[e] * h + (-sigma_hat * Ee_y[-1] - delta - lambda) * Ee_y[e] * h +
                              lambda * h;
                }
                if (ii == 0)
